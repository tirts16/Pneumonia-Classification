...
from ibmfl.model.keras_fl_model import KerasFLModel
from ibmfl.exceptions import NotFoundException, HyperparamsException
    ...
    def fusion_collected_responses(self, lst_model_updates, key='gradients'):
        ...
        if not self.b < len(lst_model_updates):
            raise HyperparamsException(
                'zeno\'s parameter of b should be less than the number of parties')

        for update in lst_model_updates:
            v.append(np.array(update.get(key)))

        if isinstance(self.fl_model, KerasFLModel):

            orignal_weights = np.array(self.current_model_weights.copy())

            indices = np.random.choice(
                self.x_train.shape[0], self.zeno_batch, replace=False)
            x_batch, y_batch = self.x_train[indices], self.y_train[indices]
            orig_loss_value = self.fl_model.evaluate_model(x_batch, y_batch)[
                'loss']
            ...
            for grads in v:
                ...
                scores.append(orig_loss_value - self.fl_model.evaluate_model(x_batch, y_batch)['loss']
                              - self.rho * norm_squared)

            self.fl_model.update_model(
                ModelUpdate(weights=orignal_weights.tolist()))

            # compute the mean update while ignoring the updates from 'b' parties with lowest scores
