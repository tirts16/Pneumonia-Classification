...
class PFNMFusionHandler(FusionHandler):
    ...
    def __init__(self, hyperparams, protocol_handler,
        ...
        self.termination_accuracy = hyperparams['global']['termination_accuracy']
    ...
    def reach_termination_criteria(self, curr_round):
        ...
        if self.data_handler is not None and self.fl_model is not None:
            ...
            if 'acc' in eval_results:
                ...
                logger.info('Global model accuracy: %s',
                            str(eval_results['acc']))
                self.global_accuracy = eval_results['acc']

                if eval_results['acc'] > self.termination_accuracy:
                    logger.info(
                        'Early termination reached after %s rounds', str(curr_round))
                    return True
            else:
    ...
    def fusion_collected_responses(self, lst_model_updates, assignment_old=None):
        """
        ...
        self._local_weights = localnet_weights
